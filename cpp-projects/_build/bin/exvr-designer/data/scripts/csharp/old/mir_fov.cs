...
namespace Ex{
    ...
    public class MriFovComponent : BaseCompiledCSharpComponent{

        CubeComponent screen;

        // ### Ex component flow functions, uncomment to use
        // # main functions
        public override bool initialize() {

            screen = get<CubeComponent>("screen");
        ...
        public override void update() {
            var distance = current_config().get<float>("distance");
            var vFov     = current_config().get<float>("vertical FOV");
            var hFov     = current_config().get<float>("horizontal FOV");
            var eyeTr    = ExVR.VR().bothEyesCamera.transform;
            
            screen.transform.position = eyeTr.position + eyeTr.forward * distance;
            screen.transform.rotation = eyeTr.rotation;

            screen.transform.localScale = new Vector3(
                2f * (Mathf.Tan(Mathf.Deg2Rad * hFov * 0.5f)) * distance,
                2f * (Mathf.Tan(Mathf.Deg2Rad * vFov * 0.5f)) * distance,
                0.001f
            );
        }
