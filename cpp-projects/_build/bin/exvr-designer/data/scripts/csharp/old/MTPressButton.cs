...
namespace Expy{
    ...
    public class MTPressButtonComponent : BaseCompiledCSharpComponent{
        ...
        public void start_routine() {
            ...
            Static.Log.message(logger.name);
        }
        ...
        public void keyboard_event(Dictionary<KeyCode, Expy.ButtonState> keys) {
            ...
            if (!buttonPressed) {
                if (keys[KeyCode.Keypad5].firstFrameDown) {
                    buttonPressed = true;
                    string line = p.currentCondition.name() + " - button pressed : " + ExComponent.ellapsed_time_routine_ms();
                    Static.Log.message(line);
                    logger.write(line, true);
                }
            } else {
                if (keys[KeyCode.Keypad5].firstFrameUp) {
                    string line = p.currentCondition.name() + " - button released : " + ExComponent.ellapsed_time_routine_ms();
                    Static.Log.message(line);
                    logger.write(line, true);
                    next();
                }
