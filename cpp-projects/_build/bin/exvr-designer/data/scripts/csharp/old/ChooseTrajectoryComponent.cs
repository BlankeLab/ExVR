...
namespace Expy{
    ...
    public class ChooseTrajectoryComponent : BaseCompiledCSharpComponent{
        ...
        public void keyboard_event(Dictionary<KeyCode, Expy.ButtonState> keys) {
            ...
            var slider = (SliderCameraComponent)Static.Components.get("Slider camera");

            if (keys[KeyCode.Keypad4].firstFrameDown) {
                Static.Log.message("Left");

                int min   = (int)slider.c.get<float>("min");
                int value = (int)slider.c.get<float>("value");
                int max   = (int)slider.c.get<float>("max");
                
                value = Math.Min(Math.Max(value - 1, min), max);
                slider.set_value(value);

            } else if (keys[KeyCode.Keypad6].firstFrameDown) {
                Static.Log.message("Right");
            ...
            } else if (keys[KeyCode.Keypad2].firstFrameDown) {
                
                int value = (int)slider.c.get<float>("value");
                Static.Log.message("Validate :" + "traj" + value);
                p.modify_routine_action_config("Move choosen", "standard", "Camera trajectory file", "traj" + value);
                next();
            }
        }
    }
