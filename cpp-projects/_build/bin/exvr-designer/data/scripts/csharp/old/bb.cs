...
namespace Ex{
    ...
    public class TemplateComponent : BaseCompiledCSharpComponent{
        ...
        //public struct KeyboardButtonState {
        //    public KeyCode code;
        //    public bool firstFrameDown;
        //    public bool firstFrameUp;
        //    public bool pressed;
        ...
        public override void slot1(object value) {
            ...
            var button = (KeyboardButtonState)value;



            if (button.code == UnityEngine.KeyCode.Alpha5) {
                // ...
            }else if(button.code == KeyCode.Keypad6) {
                // ...
            } else {
                // ...
            }

            string id = frame_id().ToString();
            // tu peux utiliser un switch aussi
            switch (button.code) {
                case KeyCode.Keypad6:
                    // ... 
                    if (button.state == KeyboardButtonState.State.Pressed) {
                        // current button is pressed
                        log_message("pressed " + id); 
                    }
                    if (button.state == KeyboardButtonState.State.Down) {
                        // only true during the first frame when the button is pressed
                        log_message("down " + id);
                    }
                    if (button.state == KeyboardButtonState.State.Up) {
                        // only true during one frame when the button is released
                        log_message("up " + id);
                    }
