...
namespace Ex{
    ...
    public class TestCamera : BaseCompiledCSharpComponent{
        ...
        TorusComponent torus;

        public override void start_experiment() {
            cube = get<CubeComponent>("Cube");
            sphere = get<SphereComponent>("Sphere");
            torus = get<TorusComponent>("Torus");

            
        }
        // public override void update() {}
        public override void start_routine() {
            var name = current_routine().name;
            var vrm = ExVR.VR().cameras();
            if (name == "C1") {

                log_message("cube position: " + cube.transform.position);
                //vrm.match_target_with_transform_by_setting_camera_rig(cube.transform, Vector3.zero, Quaternion.identity);
                ExVR.VR().cameras().set_start_experiment_neutral_transform_by_modifying_camera_rig_transform(cube.transform.position, cube.transform.rotation);
                log_message("after cube position: " + cube.transform.position);
