...
namespace Expy{
    ...
    public class QSliderComponent : BaseCompiledCSharpComponent{

        LoggerComponent logger = null;
        SliderCameraComponent slider = null;
        ...
        public void start_routine() {
            logger = (LoggerComponent)Static.Components.get("Logger");
            slider = (SliderCameraComponent)Static.Components.get("Slider camera");
            slider.set_value(4);
        }
        ...
        public void keyboard_event(Dictionary<KeyCode, Expy.ButtonState> keys) {
            ...
            if (keys[KeyCode.Keypad4].firstFrameDown) {
                if(v > 1.5f) {
                    slider.set_value(v - 1.0f);
                }
            }

            if (keys[KeyCode.Keypad6].firstFrameDown) {
            ...
            if (keys[KeyCode.Keypad5].firstFrameDown) {

                string line = p.currentCondition.name() + " - validated : " + slider.get_value();
                Static.Log.message(line);
