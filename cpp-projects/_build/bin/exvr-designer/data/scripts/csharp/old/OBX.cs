...
namespace Ex{
    ...
    public class OBXComponent : BaseCompiledCSharpComponent{

        SceneScanerComponent scaner = null;
        Vector3 hexaInitPosition = Vector3.zero;
        TransformValue scanerInitTransform = null;
        ...
        public override bool initialize() {
            ...
            // OBXComponent
            scaner = get<SceneScanerComponent>("Scene scaner");
            scanerInitTransform = scaner.configs[0].get_transform("global_transform");
            //scaner.

            log_message("Init scaner tr:  " + scanerInitTransform.position);
        ...
        public override void slot1(object value) { 
            if(current_config().name == "init_platform") {
                TransformValue tr = (TransformValue)value;
                hexaInitPosition = tr.position;
            } else if (current_config().name == "moving_platform") {
                TransformValue tr = (TransformValue)value;
                Vector3 diffPosition = tr.position - hexaInitPosition;

                TransformValue newTr = scanerInitTransform;
                newTr.position += new Vector3(0, diffPosition.y, 0);
                scaner.current_config().set_transform("global_transform", newTr);
                scaner.update_from_current_config();
            } 
        }
