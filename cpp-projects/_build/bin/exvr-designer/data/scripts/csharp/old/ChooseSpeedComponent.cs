...
namespace Expy{
    ...
    public class ChooseSpeedComponent : BaseCompiledCSharpComponent{
        ...
        public void keyboard_event(Dictionary<KeyCode, Expy.ButtonState> keys) {
            ...
            var camTraj = (CameraTrajectoryFileComponent)Static.Components.get("Camera trajectory file");

            if (keys[KeyCode.Keypad4].firstFrameDown) {
                Static.Log.message("Left");


                float min = slider.c.get<float>("min");
                float value = slider.c.get<float>("value");
                float max = slider.c.get<float>("max");

                value = Math.Min(Math.Max(value - 1, min), max);
                slider.set_value(value);

                foreach (ComponentConfig config in camTraj.configs) {
                    config.set("global_time", value);
                }
