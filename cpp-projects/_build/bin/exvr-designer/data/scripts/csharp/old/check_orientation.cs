...
namespace Expy{
    ...
    public class CheckOrientation : BaseCompiledCSharpComponent{
        ...
        public bool initialize() {
            log("initialize CheckOrientation");
            return true;
        }

        public void start_experiment() {
            logger = (LoggerComponent)Component.get("Logger");
        }
            ...
            foreach (var axe in axes) {
                ...
                if (axe.Key == "DPadX_P1") {
                    if (axe.Value != 0f) {
                        ...
                        logger.write("Time " + Component.ellapsed_time_exp_ms() + " - " 
                                             + Component.get("3D rod image").conditionName + " : "
                                             + ((axe.Value < 0f) ?  "Left" : "Right"), true);                        
                        log("next");
                        next();
                        return;
                    }
